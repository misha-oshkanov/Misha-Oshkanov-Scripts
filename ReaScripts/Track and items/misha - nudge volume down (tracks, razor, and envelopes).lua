-- @description Smart nudge volume up. Works on tracks and automation items via selecting, items and envelopes via razor or selecting
        ...
        if area ~= "" then
            ...
            for ir,rz in ipairs(razors) do      
                ...
                if mode == 'rz_item' then
                    retval, deffadelen = reaper.get_config_var_string( 'defsplitxfadelen' )
                    ...
                    for _, cut_item in ipairs(items_tocut) do 
                        ...
                        elseif cut_item.mode == 'right' then 
                            split_item = reaper.SplitMediaItem(cut_item.item, rz.end_time-deffadelen)

                            reaper.SetMediaItemInfo_Value(split_item, 'D_VOL', db2val(val2db(reaper.GetMediaItemInfo_Value(cut_item.item, 'D_VOL'))+add) )
                        elseif cut_item.mode == 'both' then 
                            split_item = reaper.SplitMediaItem(cut_item.item, rz.start_time)
                            reaper.SplitMediaItem(split_item, rz.end_time-deffadelen)
                            reaper.SetMediaItemInfo_Value(split_item, 'D_VOL', db2val(val2db(reaper.GetMediaItemInfo_Value(cut_item.item, 'D_VOL'))+add) )
                        end
                    end
