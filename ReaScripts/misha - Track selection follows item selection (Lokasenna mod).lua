...
Version: 1.1.3
Author: Lokasenna, Misha Oshkanov
  ...
  if shallow_equal(sel_tracks, cur_tracks) then
    ...
    if not shallow_equal(sel_items, cur_items) then
      ...
      if #tracks > 0 then
        ...
        -- Update sel_tracks to match what we just selected,
        -- so the next iteration doesn't enter the else branch
        -- and miss item changes.
        sel_tracks = {}
