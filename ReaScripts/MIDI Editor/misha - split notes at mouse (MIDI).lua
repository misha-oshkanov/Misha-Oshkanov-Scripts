...
function split_selected(take, cur_sel_note_idx, mouse_pos)
  ...
  while cur_sel_note_idx ~= -1 do
    ...
    if startppqpos < mouse_pos and endppqpos > mouse_pos then
      reaper.MIDI_SetNote(take, cur_sel_note_idx, false, muted, startppqpos, mouse_pos, chan, pitch, vel, true)
...
function main()
  ...
  for i=1, reaper.CountSelectedMediaItems(0) do 
      ...
      itemtake = reaper.GetActiveTake(item)
      if reaper.TakeIsMIDI(itemtake) == true then 
          table.insert(takes, itemtake)
      end
  end
