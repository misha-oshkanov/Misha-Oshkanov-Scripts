...
-- 0 = Top, 1 = Bottom, 2 = In line (in the main row).
function apply_free_mode_pos()
    FREE_MODE_TOP    = FREE_MODE_POS == 0
    FREE_MODE_INLINE = FREE_MODE_POS == 2
end
...
function LoadSettings()
    ...
    if saved_pos == nil then
        -- Migrate the old separate checkboxes: top -> 0, bottom -> 1, in line -> 2
        saved_pos = get_bool('FREE_MODE_INLINE', false) and 2
            or (get_bool('FREE_MODE_TOP', false) and 0 or 1)
    end
    FREE_MODE_POS = saved_pos
    apply_free_mode_pos()
...
-- Locate FTC Tools/Adaptive grid by walking up from the script's folder,
-- so the controller works no matter how deep it is installed
-- (was a fixed '../..' climb that only matched a 2-level-deep install).
local grid_adapt_script, grid_service_script
do
    local dir = grid_dir
    while dir and dir ~= '' do
        local adapt = gpath(dir, 'FTC Tools', 'Adaptive grid', 'Adapt grid to zoom level.lua')
        local service = gpath(dir, 'FTC Tools', 'Adaptive grid', 'Adaptive grid (background service).lua')
        if reaper.file_exists(adapt) and reaper.file_exists(service) then
            grid_adapt_script, grid_service_script = adapt, service
            break
        end
        dir = dir:match('^(.+)[\\/]') -- strip one path component (nil at drive root)
    end
end
local grid_has_adaptive = grid_adapt_script ~= nil
grid_text = '1/4'
grid_hovered = false
...
local function GridRunAdapt(is_midi)
    if not grid_adapt_script then return end
...
local function GridStartService()
    if not grid_service_script or not reaper.file_exists(grid_service_script) then return end
...
local function GridUpdateText()
    ...
    elseif s == 3 then
        ...
        if is_t then
            suf = 'T'; denom = denom * 2 / 3
        elseif is_q then
            suf = 'Q'; denom = denom * 4 / 5
        elseif is_s then
            suf = 'S'; denom = denom * 4 / 7
        elseif is_do then
            suf = 'D'; denom = denom / 2; num = num / 3
        end
        if num > 1 then
            local rest = denom % num
            if rest == 0 then
                denom = denom / num; num = 1
            end
        end
...
function draw_volume_buttons(master, w)
    for i, b in ipairs(buttons) do
        ...
        if b_button then
            ...
            if reaper.TrackFX_GetOpen(master, index + (0x1000000)) then reaper.TrackFX_Show(master, index + (0x1000000),
                    0) end
...
function draw_correction_single_button(w)
    ...
    if ImGui.BeginPopup(ctx, 'corr_popup_menu') then
        ...
        if fx_container ~= -1 then
            ...
            for i = 0, count - 1 do
                local _, item = reaper.TrackFX_GetNamedConfigParm(master, fx_container + (0x1000000),
                    "container_item." .. i)
                ...
                if ImGui.Selectable(ctx, clean_name .. "##" .. i, is_selected) then
                    for j = 0, count - 1 do
                        local _, other_item = reaper.TrackFX_GetNamedConfigParm(master, fx_container + (0x1000000),
                            "container_item." .. j)
...
function draw_free_mode_slider(master, w)
    ...
    if vertical ~= 0 then
        ...
        if is_ctrl then
            ...
            local step = math.floor(((math.log(slider_range) - math.log(min_hz)) * 100 / (math.log(max_hz) - math.log(min_hz))) /
            scroll_accuracy)
    ...
    for bi = 1, 5 do
        ...
        local x0 = min_x + (bi - 1) * seg_w

        reaper.ImGui_DrawList_AddRectFilled(draw_list, x0, min_y, x0 + seg_w, max_y,
    ...
    reaper.ImGui_DrawList_AddRectFilled(draw_list, low_x, min_y, high_x, max_y, rgba(200, 200, 200, 0.4), 2)
...
function draw_listen_buttons(master, w)
    ...
    -- the exact footprint of those five buttons (5 x w + their 4 x 2px gaps),
    -- so the Free button and the total row width stay exactly where they were.
    local inline = free_mode and FREE_MODE_INLINE

    if inline then
        draw_free_mode_slider(master, w * 5 + 2 * 4)
        reaper.ImGui_SameLine(ctx)
    end

    for i2, lb in ipairs(listen_buttons) do
        if not inline or lb.str == 'Free' then
            ImGui.PushID(ctx, i)
            ImGui.PushFont(ctx, font2, font_size2)
            ...
            if ext == i2 then
                ...
                ImGui.PushStyleColor(ctx, ImGui.Col_ButtonActive(), rgba(lb.col[1] + 10, lb.col[2] + 10, lb.col[3] + 10,
                    lb.col[4]))
    ...
    if visible then
        if reaper.ImGui_BeginTable(ctx, "LayersTable", 3, reaper.ImGui_TableFlags_BordersInnerV()) then
            ...
            reaper.ImGui_TableSetupColumn(ctx,
                "Layer 2" .. (current_layer == 2 and " [Active]" or "") .. (MAX_LAYERS == 1 and " [OFF]" or ""))
            reaper.ImGui_TableSetupColumn(ctx,
                "Layer 3" .. (current_layer == 3 and " [Active]" or "") .. (MAX_LAYERS <= 2 and " [OFF]" or ""))
            ...
            for i = 1, #layers do
                ...
                if reaper.ImGui_Checkbox(ctx, "Volume##" .. i, l.vol) then
                    l.vol = not l.vol; should_resize = true; SaveSettings()
                end
            end
            ...
            for i = 1, #layers do
                ...
                if i == current_layer then a = 0.4 else a = 0.2 end
                reaper.ImGui_TableSetBgColor(ctx, reaper.ImGui_TableBgTarget_CellBg(), rgba(c.r, c.g, c.b, a))
                if reaper.ImGui_Checkbox(ctx, "Corrections##" .. i, l.corr) then
                    l.corr = not l.corr; should_resize = true; SaveSettings()
                end
            end
            ...
            for i = 1, #layers do
                ...
                if i == current_layer then a = 0.4 else a = 0.2 end
                reaper.ImGui_TableSetBgColor(ctx, reaper.ImGui_TableBgTarget_CellBg(), rgba(c.r, c.g, c.b, a))
                if reaper.ImGui_Checkbox(ctx, "Refs##" .. i, l.ref) then
                    l.ref = not l.ref; should_resize = true; SaveSettings()
                end
            end
            ...
            for i = 1, #layers do
                ...
                if i == current_layer then a = 0.4 else a = 0.2 end
                reaper.ImGui_TableSetBgColor(ctx, reaper.ImGui_TableBgTarget_CellBg(), rgba(c.r, c.g, c.b, a))
                if reaper.ImGui_Checkbox(ctx, "AB from Metric##" .. i, l.ab) then
                    l.ab = not l.ab; should_resize = true; SaveSettings()
                end
            end
            ...
            for i = 1, #layers do
                ...
                if i == current_layer then a = 0.4 else a = 0.2 end
                reaper.ImGui_TableSetBgColor(ctx, reaper.ImGui_TableBgTarget_CellBg(), rgba(c.r, c.g, c.b, a))
                if reaper.ImGui_Checkbox(ctx, "Item Count##" .. i, l.item_count) then
                    l.item_count = not l.item_count; should_resize = true; SaveSettings()
                end
            end
            ...
            for i = 1, #layers do
                ...
                if i == current_layer then a = 0.4 else a = 0.2 end
                reaper.ImGui_TableSetBgColor(ctx, reaper.ImGui_TableBgTarget_CellBg(), rgba(c.r, c.g, c.b, a))
                if reaper.ImGui_Checkbox(ctx, "Grid Box##" .. i, l.grid) then
                    l.grid = not l.grid; should_resize = true; SaveSettings()
                end
            end
            ...
            for i = 1, #layers do
                ...
                if i == current_layer then a = 0.4 else a = 0.2 end
                reaper.ImGui_TableSetBgColor(ctx, reaper.ImGui_TableBgTarget_CellBg(), rgba(c.r, c.g, c.b, a))
                if reaper.ImGui_Checkbox(ctx, "Meter##" .. i, l.mtr) then
                    l.mtr = not l.mtr; should_resize = true; SaveSettings()
                end
            end
            ...
            for i = 1, #layers do
                ...
                if i == current_layer then a = 0.4 else a = 0.2 end
                reaper.ImGui_TableSetBgColor(ctx, reaper.ImGui_TableBgTarget_CellBg(), rgba(c.r, c.g, c.b, a))
                if reaper.ImGui_Checkbox(ctx, "Monitoring FX##" .. i, l.monfx) then
                    l.monfx = not l.monfx; should_resize = true; SaveSettings()
                end
            end
        ...
        if reaper.ImGui_TreeNode(ctx, "Advanced Settings") then
            ...
            local changed_mon, new_mon = reaper.ImGui_Checkbox(ctx, "Metric AB in Monitoring FX",
                USE_METRIC_IN_MONITORINGFX)
            if changed_mon then
            ...
            local rv_sc, new_sc = reaper.ImGui_SliderDouble(ctx, "Scroll Speed in free mode", scroll_accuracy, 0.1, 5.0,
                "%.1f")
            if rv_sc then
                scroll_accuracy = new_sc; SaveSettings()
            end
            ...
            if rv_gw then
                grid_width = new_gw; should_resize = false; SaveSettings()
            end
            ...
            if changed_dco then
                DISABLE_CORR_ON_START = dco; SaveSettings()
            end

            local free_mode_pos_txt = { [0] = 'Top', [1] = 'Bottom', [2] = 'In line' }
            local changed_fmp, fmp = reaper.ImGui_SliderInt(ctx, "Free mode slider position", FREE_MODE_POS, 0, 2,
                free_mode_pos_txt[FREE_MODE_POS] or '?')
            if changed_fmp then
                FREE_MODE_POS = fmp
                apply_free_mode_pos()
                should_resize = false; SaveSettings()
            end
...
function mtr_update()
    ...
    if mtr_d1 > mtr_pk1 or now - mtr_t1 >= MTR_HOLD_TIME then
        mtr_pk1 = mtr_d1; mtr_t1 = now
    end
    if mtr_d2 > mtr_pk2 or now - mtr_t2 >= MTR_HOLD_TIME then
        mtr_pk2 = mtr_d2; mtr_t2 = now
    end
